// Weather Station - Test Version (ohne Windsensor, TSL2591, Rain Reed Sensor)
// Version: 2.0 Test (Updated with I2C Multiplexer and CCS811 Air Quality Sensor)
// Features: BME280, MLX90614, CCS811, Rain Drop Sensor, nRF24L01
...
void setup() {
  ...
  Serial.println(F("Weather Station Test Version Starting..."));
  Serial.println(F("Version 2.0 Test - Ohne Windsensor, TSL2591, Rain Reed Sensor"));

  // Initialize rain drop sensor pin
  ...
  Serial.println(F("Weather Station Test initialization complete!"));
  Serial.println(F("Verfügbare Sensoren: BME280, MLX90614, CCS811, Rain Drop Sensor"));
}
...
void loop() {
  ...
  setDefaultValuesForMissingSensors();
...
// Set default values for missing sensors
void setDefaultValuesForMissingSensors() {
  // Windsensor nicht angeschlossen
  results[6] = 0; // Wind revolutions = 0 (kein Wind)
  
  // TSL2591 nicht angeschlossen
  results[4] = 0; // Light sensor = 0 (kein Licht)
  
  // Rain Reed Sensor nicht angeschlossen
  results[5] = 0; // Rain tips = 0 (kein Regen)
}
...
void printDebugInfo() {
  ...
  // Print rain detection debug info (nur Rain Drop Sensor)
  int rainDropAnalogVal = analogRead(RAIN_DROP_ANALOG_PIN);
  int rainDropDigitalVal = digitalRead(RAIN_DROP_DIGITAL_PIN);
  ...
  // Print sensor status
  Serial.print(F("Sensors - BME280: "));
  Serial.print(bme_sensor_available ? F("OK") : F("ERROR"));
  Serial.print(F(", MLX90614: "));
  Serial.print(mlx_sensor_available ? F("OK") : F("ERROR"));
  Serial.print(F(", CCS811: "));
  Serial.print(ccs811_sensor_available ? F("OK") : F("ERROR"));
  Serial.println();
  
  // Print missing sensors info
  Serial.println(F("Missing sensors: Windsensor, TSL2591, Rain Reed Sensor"));
}
